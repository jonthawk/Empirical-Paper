...
class Moments:
    ...
    def __init__(self, Data, K, C,
        ...
        self.params     = [1 for k in range(self.K+1)]
    ...
    def compute_P0(self, params):
        P0 = np.zeros((self.T, self.M, self.NS, self.K+1))
        for t in range(self.T):
            for mkt in range(self.M):
                for i in range(self.NS):
                    for k in range(self.K+1):
                        P0[t][mkt][i][k] = self.normals[t][mkt][i][k]*params[k]
    ...
    def simulate_shares(self, delta, t, mkt):
        ...
        for i in range(self.NS):
            totals = np.add(totals, self.cond_choice_prob(delta, self.P0[t][mkt][i], t, mkt))
